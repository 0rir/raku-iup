...
sub MAIN() {
    ...
    my $frame1 = $ih.frame(
        $ih.hbox(
            $ih.fill(),
            $ih.button("1", "").set-attrs(SIZE => "30x30"),
            $ih.button("2", "").set-attrs(SIZE => "30x40"),
            $ih.button("3", "").set-attrs(SIZE => "30x50"),
            $ih.fill().set-attrs(ALIGNMENT => "ATOP", GAP => "10",
                    SIZE => "200")
            # Sets hbox's alignment, gap and size
            )
        );
    $frame1.set-attribute("TITLE", "ALIGNMENT=ATOP, GAP=10, SIZE=200");

    # Creates frame with three buttons */
    my $frame2 = $ih.frame(
        $ih.hbox(
            $ih.fill(),
            $ih.button("1", "").set-attributes(SIZE => "30x30"),
            $ih.button("2", "").set-attributes(SIZE => "30x40"),
            $ih.button("3", "").set-attributes(SIZE => "30x50"),
            $ih.fill().set-attributes(ALIGNMENT => "ACENTER",
                GAP => "20")
    ...
    my $frame3 = $ih.frame(
        $ih.hbox(
            ...
            $ih.button("2", "").set-attrs(SIZE => "30x40"),
            $ih.button("3", "").set-attrs(SIZE => "30x50"),
            $ih.fill.set-attrs(ALIGNMENT => "ABOTTOM", SIZE => "150")
